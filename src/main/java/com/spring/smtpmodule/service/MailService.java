...
//        thymeleafContext.setVariable("content", content); //예약 관련 내용
    ...
    //html/jsp로 메일 보내는 경우
    public void sendMailJsp(String mailType, List<String> userMail, List<String> userName, Long meetRoom, LocalDateTime meetTime, String meetTitle, String meetContent){
        List<String> userEmailList = new ArrayList<>(); //참여자 이메일 리스트
        List<String> userList = new ArrayList<>(); //참여자 리스트
        LocalDate reservateDate = meetTime.toLocalDate(); //회의 날짜
        LocalTime reservateTime = meetTime.toLocalTime(); //회의 시간
        int hour = reservateTime.getHour(); //시간
        String minute = String.format("%02d", reservateTime.getMinute()); //분
        ...
        try {
            ...
            String htmlContent = "<div style='width:500px;'>"
                    ...
                    + "<p style='margin-block:10px;'>" + "<strong>회의 날짜</strong>&nbsp;&nbsp;&nbsp;&nbsp;" + reservateDate + "</p>"
                    + "<p style='margin-block:10px;'>" + "<strong>회의 시간</strong>&nbsp;&nbsp;&nbsp;&nbsp;" + hour + "시 " + minute + "분" + "</p>"
